...
# self.vis2 = visdom.Visdom(env='att_main')
        ...
        self.Mod = torch.nn.ModuleList([ST_Block5(args.n_resblocks, nFeat, i) for i in range(args.n_resgroups)])  # nn.ModuleList()
        ...
        if scale == 4:
            self.up = nn.Sequential(nn.Conv2d(nFeat, nFeat * scale, kernel_size=3, padding=1, bias=True),
                                    sub_pixel(2), nn.Conv2d(nFeat, nFeat * scale, kernel_size=3, padding=1, bias=True),
                                    sub_pixel(2))
        else:
            self.up = nn.Sequential(nn.Conv2d(nFeat, nFeat * scale * scale, kernel_size=3, padding=1, bias=True),
                                    sub_pixel(2))

        self.HR = nn.Sequential(nn.Conv2d(nFeat, nFeat, kernel_size=3, padding=1, bias=True),nn.LeakyReLU(0.1),
    ...
    def forward(self, x):
        ...
        if self.plot_cnt == 200 and s is not None:
            ...
            for idx, sle in enumerate(s):
                self.id[idx] = self.vis2.images(sle[0, :, :, :].unsqueeze(dim=1), nrow=6, win=self.id[idx],
                                                opts={'title': 'BLOCK-15'})
...
class ST_Unit5(nn.Module):
    def __init__(self, nChannel, prev=0, bias=True, dilation=1,conv1=None,conv2=None):  # ch = 128  nch = 64 (split)
...
class ST_Block5(nn.Module):
    def __init__(self, num_unit, nChannel, prev=0, bias=True):  # ch = 128  nch = 64 (split)
        ...
        self.Fuse = nn.Conv2d(nChannel*2, nChannel, kernel_size=1, padding=0, bias=bias)  # *(num_unit+1)
        self.Infuse = nn.Conv2d(nChannel * (prev + 1), nChannel, kernel_size=1, padding=0, bias=bias)
        if prev > 0: self.Conv = nn.Conv2d(nChannel * (2), nChannel, kernel_size=1, padding=0, bias=bias)
...
if __name__ == '__main__':
    C = nn.Conv2d(64, 64, kernel_size=3, padding=1, groups=8)
